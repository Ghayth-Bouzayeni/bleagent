...
async def upsert_tag_state(
    ...
    Args:
        db: Database session
        beacon_uuid: BLE Beacon UUID (stable identifier)
        beacon_major: iBeacon Major value
        beacon_minor: iBeacon Minor value
        lat: Latitude
        lon: Longitude
    ...
    stmt = select(TagState).where(TagState.beacon_uuid == beacon_uuid)
...
async def get_tag_state(db: AsyncSession, beacon_uuid: str) -> TagState | None:
    """
    Get tag state by Beacon UUID
    
    Args:
        db: Database session
        beacon_uuid: BLE Beacon UUID
